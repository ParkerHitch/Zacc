...
pub fn build(b: *std.Build) void {
    ...
    const specTest = b.addTest(.{ .root_source_file = b.path("src/lang/newSpec.zig"), .target = target, .optimize = optimize });
    specTest.root_module.addImport("compilerLib", mod);
    const runSpecTest = b.addRunArtifact(specTest);
    const specTestStep = b.step("specTest", "asd");
    specTestStep.dependOn(&runSpecTest.step);
}
